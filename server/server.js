...
app.get("/players/:id", async (req, res) => {
    ...
    try {
        ...
        const q3 = db.query("select sum(runs_scored) as runs, 4*count(case when runs_scored = 4 then 1 end) as fours, 6*count(case when runs_scored = 6 then 1 end) as sixes from ball_by_ball where striker = $1", [req.params.id])
