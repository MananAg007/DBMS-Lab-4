...
app.get("/venues/:id", async (req, res) => {
    ...
    try {
        ...
        const q3 = db.query("With res as (select match.match_id, innings_no, sum(runs_scored + extra_runs) as runs from ball_by_ball, match where ball_by_ball.match_id = match.match_id and match.venue_id = $1 group by match.match_id, innings_no) select coalesce(max(runs),0) as highest, coalesce(min(runs),0) as lowest from res;", [req.params.id])
...
app.get("/players/:id", async (req, res) => {
    ...
    try {
        ...
        const q8 = db.query("select count(case when out_type is not null and out_type not in ('run out','retired hurt') then 1 end), sum(runs_scored + extra_runs), match_id from ball_by_ball where bowler = $1 group by match_id;", [req.params.id]);
